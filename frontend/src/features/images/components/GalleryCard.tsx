...
export const GalleryCard = ({
  ...
  return (
    <button
      type="button"
      className="masonry-item group relative block w-full min-h-[220px] sm:min-h-[240px] overflow-hidden rounded-[12px] bg-surface-dark shadow-lg shadow-black/25 ring-1 ring-white/5 transition-transform duration-300 hover:-translate-y-1"
      style={{ aspectRatio }}
      onClick={onSelect}
    >
      {image.image_url ? (
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 translate-y-4 transition-transform duration-300 group-hover:translate-y-0">
          ...
          <div className="flex flex-wrap items-center justify-end gap-2">
            <button
              ...
              className="pointer-events-auto flex size-9 items-center justify-center rounded-full bg-white/10 text-white backdrop-blur-sm transition-colors hover:bg-white/20"
