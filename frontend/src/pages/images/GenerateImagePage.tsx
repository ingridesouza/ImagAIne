...
export const GenerateImagePage = () => {
  ...
  const presetPrompt = locationState?.prompt ?? locationState?.promptDraft ?? '';
  const presetNegativePrompt = locationState?.negative_prompt ?? '';
  const shouldAutoGenerate = locationState?.autoGenerate ?? false;
  ...
  useEffect(() => {
    ...
    if (presetNegativePrompt) {
      setValue('negative_prompt', presetNegativePrompt, { shouldValidate: true, shouldDirty: false });
    }
  }, [presetPrompt, presetNegativePrompt, setValue]);
  ...
  // Auto-generate when coming from Prompt Assistant with autoGenerate flag
  useEffect(() => {
    if (shouldAutoGenerate && presetPrompt && !hasAutoGenerated && !isPending) {
      setHasAutoGenerated(true);
      const payload: GenerateImagePayload = {
        prompt: presetPrompt,
        negative_prompt: presetNegativePrompt || undefined,
        aspect_ratio: '1:1',
      };
      void mutateAsync(payload);
    }
  }, [shouldAutoGenerate, presetPrompt, presetNegativePrompt, hasAutoGenerated, isPending, mutateAsync]);
